...
def predict_ridge_quarter_returns(df: pd.DataFrame, alpha: float = 1.0) -> tuple[pd.DataFrame, pd.Series]:
    df_sorted = df.sort_values("index")
    ignore_cols = ["index", "ticker", "period_quarter", "Quarterly Return"]
    feature_cols = [col for col in df.columns if col not in ignore_cols]
...
def predict_lasso_quarter_returns(df: pd.DataFrame, alpha: float = 1.0) -> tuple[pd.DataFrame, pd.Series]:
    df_sorted = df.sort_values("index")
    ignore_cols = ["index", "ticker", "period_quarter", "Quarterly Return"]
    feature_cols = [col for col in df.columns if col not in ignore_cols]
